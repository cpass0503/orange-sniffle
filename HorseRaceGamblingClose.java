...
public class HorseRaceGamblingClose {
    public static void main(String[] args) {
        // Create horses
        List<Horse> horses = new ArrayList<>();
        horses.add(new Horse("Splashy", 4.0));   // 25% chance of winning
        horses.add(new Horse("Thunder", 5.0));   // 20% chance of winning
        horses.add(new Horse("Blaze", 6.67));    // 15% chance of winning
        horses.add(new Horse("Storm", 8.33));    // 12% chance of winning
        horses.add(new Horse("Flash", 10.0));    // 10% chance of winning
        horses.add(new Horse("Bolt", 12.5));     // 8% chance of winning
        horses.add(new Horse("Whirlwind", 16.67)); // 6% chance of winning
        horses.add(new Horse("Gale", 25.0));     // 4% chance of winning
...
private static Horse runGraphicalRace(List<Horse> horses) {
    ...
    int closeRaceLength = (int) (trackLength * 0.8); // 80% of the race will be close
    ...
    while (!raceFinished) {
        ...
        for (int i = 0; i < horses.size(); i++) {
            // Early phase: all horses move more or less similarly (close race)
            if (positions[i] < closeRaceLength) {
                positions[i] += rand.nextInt(2); // All horses move 0 or 1 steps randomly
            } else {
                // Final phase: odds now influence the movement (last 20%)
                ...
                if (rand.nextDouble() < probabilityOfMove) {
                    positions[i] += rand.nextInt(3); // Move the horse 0 to 2 steps
                }
            }
